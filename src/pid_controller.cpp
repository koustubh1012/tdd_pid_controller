...
PIDController::PIDController(double robot_x, double robot_y, double d_t,
                             double thresh) {
...
// Move the robot based on calculated velocities (this should implement PID
// control logic)
...
void PIDController::compute_velocity() {
  ...
  calculated_velocity_x =
      Kp * current_error_x + Ki * accumulated_error_x + Kd * der_errx;
  calculated_velocity_y =
      Kp * current_error_y + Ki * accumulated_error_y + Kd * der_erry;
